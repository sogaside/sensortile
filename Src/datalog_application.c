...
char newLine[] = "\r\n";
extern int16_t acctempx,acctempy,acctempz;//算法传递的数据
extern float gyrotempx,gyrotempy,gyrotempz;
/**
...
void Accelero_Sensor_Handler( void *handle )
{
  ...
  if ( status == 1 )
  {
    ...
    //  sprintf( dataOut, "\n\rACC_X: %d, ACC_Y: %d, ACC_Z: %d", (int)acceleration.AXIS_X, (int)acceleration.AXIS_Y, (int)acceleration.AXIS_Z );
        sprintf( dataOut, "%d,%d,%d,", (int)acceleration.AXIS_X, (int)acceleration.AXIS_Y, (int)acceleration.AXIS_Z );
      acctempx=(int)acceleration.AXIS_X;
		acctempy=(int)acceleration.AXIS_Y;
		acctempz=(int)acceleration.AXIS_Z;
...
void Gyro_Sensor_Handler( void *handle )
{
  ...
  if ( status == 1 )
  {
    ...
    gyrotempx=(int)angular_velocity.AXIS_X/1000.00;
		gyrotempy=(int)angular_velocity.AXIS_Y/1000.00;
		gyrotempz=(int)angular_velocity.AXIS_Z/1000.00;
